...
$(() => {
  const game4096 = {
    ...
    init: () => {
      // hide all the screens besides "start screen"
      $("#section-in-game-screen").hide();
      $("#section-instruction-screen").hide();
      $("#section-end-screen").hide();

      // set up event listeners to all viable keypresses
      $(window).keydown(e => {
        switch (e.key) {
          // cascade three cases for one code block
          case "ArrowLeft":
          case "A":
          case "a":
            if (
      ...
      $("form").submit(e => {
        ...
        if (!$("#user-name").val()) {
          ...
          // when the user input has a truthy value
          game4096.playerName = $("#user-name").val();

          $("#section-start-screen").hide();
          $("#section-in-game-screen").show();

          // generate two initial random blocks in in-game stage
          game4096.randomPopulate();
          game4096.randomPopulate();
    ...
    resetGame: () => {
      ...
      // empty out all the existing value in grid items
    ...
    checkGameOver: () => {
      ...
      // check if the grid is full (all blocks have a value)
      matrix.forEach(item => {
